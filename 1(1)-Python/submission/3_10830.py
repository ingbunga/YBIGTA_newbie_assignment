...
class Matrix:
    ...
    def __pow__(self, n: int) -> Matrix:
        # 구현하세요!
        if n == 0:
            return Matrix.eye(len(self.matrix))

        q = n // 2
        r = n % 2
    
        q_powed = self.__pow__(q)
        q_powed_square = q_powed.__matmul__(q_powed)
    
        return self.__matmul__(q_powed_square) if r else q_powed_square
