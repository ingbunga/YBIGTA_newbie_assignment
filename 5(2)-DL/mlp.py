...
class MultiLayerPerceptron(object):
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dh3 = dz3 * (1 - z3 ** 2)   # also d_alpha
        db3 = np.sum(dh3, axis=0)
        dW3 = z2.T @ dh3 + 2 * L2_norm * W3
        dz2 = dh3 @ W3.T

        dh2 = dz2 * ((z2 >= 0).astype(np.float32)*0.99 + 0.01)
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W3.T

        dh1 = dz1 * (z1 >= 0).astype(np.float32) + dh3
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            loss = self.compute_loss(y_pred=y_hat, y_true=y_train, L2_norm=L2_norm)
            grad = self.back_propagation(cache=cache, X=X_train, y=y_train, L2_norm=L2_norm)
